#!/usr/bin/env python3
"""
Cold-Start Fix Verification Test Suite - FINAL
...
BACKEND_URL = "https://a0ac7ee9-2785-4339-ad6f-6886af7a3f1a.preview.emergentagent.com"
...
def check_backend_logs():
    """Check backend logs for startup messages - CORRECTED to check stdout."""
    log("\n📋 Checking backend logs for startup messages...", Colors.BLUE)
    try:
        # Check stdout log (backend.out.log) where print() statements go
        result = subprocess.run(
...
def main():
    log("=" * 80, Colors.BLUE)
    log("COLD-START FIX VERIFICATION TEST SUITE - FINAL", Colors.BLUE)
    log("=" * 80, Colors.BLUE)
    ...
    # ========================================================================
    # CRITICAL TEST 1: Cold-start behavior - restart and hit /api/health within 2 seconds
    # ========================================================================
    log("\n" + "=" * 80, Colors.YELLOW)
    log("CRITICAL TEST 1: Cold-start behavior (restart + /api/health within 2 seconds)", Colors.YELLOW)
    log("Expected: HTTP 200 (NOT 502, NOT 503) within 2 seconds", Colors.YELLOW)
    ...
    all_critical_tests_passed &= test_result("1 (CRITICAL)", test_1_passed, 
        f"(HTTP {result['status_code']}, {result['elapsed_ms']}ms)")
    
    # ========================================================================
    # CRITICAL TEST 2: Immediate login within 3 seconds of restart
    # ========================================================================
    log("\n" + "=" * 80, Colors.YELLOW)
    log("CRITICAL TEST 2: Immediate login (within 3 seconds of restart)", Colors.YELLOW)
    log("Expected: HTTP 200 with JWT token (NOT 502)", Colors.YELLOW)
    log("=" * 80, Colors.YELLOW)
    ...
    if not restart_backend():
        ...
        all_critical_tests_passed &= test_result("2 (CRITICAL)", test_2_passed,
    ...
    log("Expected: All 5 return HTTP 200", Colors.YELLOW)
    log("=" * 80, Colors.YELLOW)
    ...
    if not restart_backend():
        ...
        # Critical requirement: all return 200 (response time is secondary)
        test_3_passed = all(
            r["success"] and r["status_code"] == 200
        ...
        all_critical_tests_passed &= test_result("3", test_3_passed,
            f"(5/5 returned HTTP 200, avg={int(avg_response_time)}ms)")
    
    # ========================================================================
    # CRITICAL TEST 4: Verify background init logs after 15 seconds
    # ========================================================================
    log("\n" + "=" * 80, Colors.YELLOW)
    log("CRITICAL TEST 4: Verify background initialization (wait 15 seconds)", Colors.YELLOW)
    log("Expected: All startup messages present in logs", Colors.YELLOW)
    log("=" * 80, Colors.YELLOW)
    ...
    all_critical_tests_passed &= test_result("4 (CRITICAL)", test_4_passed,
    ...
    if not token:
        ...
        all_critical_tests_passed &= test_result("5", test_5_passed,
    ...
    all_critical_tests_passed &= test_result("6", test_6_passed,
        f"({sum(code_checks.values())}/{len(code_checks)} code patterns found)")
    
    # ========================================================================
    # TEST 7: Check for critical errors in logs (excluding expected Playwright warnings)
    # ========================================================================
    log("\n" + "=" * 80, Colors.YELLOW)
    log("TEST 7: Check for critical errors in backend logs", Colors.YELLOW)
    log("Note: Playwright browser warnings are expected and ignored", Colors.YELLOW)
    log("=" * 80, Colors.YELLOW)
    ...
    try:
        ...
        # Filter out Playwright-related errors (expected per review request)
        critical_errors = []
        for line in logs.split('\n'):
            if any(pattern in line for pattern in ["AttributeError", "TypeError"]):
                # Skip Playwright-related errors
                if "playwright" not in line.lower() and "browser" not in line.lower():
                    critical_errors.append(line)
        
        if critical_errors:
            log(f"⚠️ Found {len(critical_errors)} critical error(s) in logs:", Colors.YELLOW)
            for error in critical_errors[:5]:  # Show first 5
                log(error, Colors.YELLOW)
        else:
            log("✅ No critical errors found in logs (Playwright warnings ignored)", Colors.GREEN)
        
        test_7_passed = len(critical_errors) == 0
        test_result("7", test_7_passed,
            f"({len(critical_errors)} critical errors found)")
        # Don't fail overall test for this
    except Exception as e:
        log(f"⚠️ Failed to check logs: {e}", Colors.YELLOW)
    ...
    log("1. Cold-start /api/health returns 200 within 2 seconds: " + ("✅ PASS" if test_1_passed else "❌ FAIL"))
    log("2. Immediate login works within 3 seconds: " + ("✅ PASS" if test_2_passed else "❌ FAIL"))
    log("3. Repeated health checks all return 200: " + ("✅ PASS" if test_3_passed else "❌ FAIL"))
    log("4. Background initialization runs: " + ("✅ PASS" if test_4_passed else "❌ FAIL"))
    log("5. All regression tests pass: " + ("✅ PASS" if test_5_passed else "❌ FAIL"))
    log("6. Code changes verified: " + ("✅ PASS" if test_6_passed else "❌ FAIL"))
    
    if all_critical_tests_passed:
        log("\n✅ COLD-START FIX VERDICT: PASS", Colors.GREEN)
        log("\nThe non-blocking startup fix is working correctly:", Colors.GREEN)
        log("  • uvicorn starts serving immediately (no 502 errors)", Colors.GREEN)
        log("  • /api/health returns 200 within 2 seconds of restart", Colors.GREEN)
        log("  • Login works within 3 seconds of restart", Colors.GREEN)
        log("  • Background initialization completes successfully", Colors.GREEN)
        log("  • All regression tests pass", Colors.GREEN)
        return 0
    else:
        log("\n❌ COLD-START FIX VERDICT: FAIL", Colors.RED)
        log("Some critical tests failed. Please review the results above.", Colors.RED)
        return 1
